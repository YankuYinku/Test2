...
$authorizationConnectionString = $(az keyvault secret show --vault-name "apetitoKeyVault" --name "apetito-authorization-api-ConnectionStrings-AuthorizationDbConnectionString-meinapetito" --query "value")
...
$notificationConnectionString = $(az keyvault secret show --vault-name "apetitoKeyVault" --name "apetito-notification-api-ConnectionStrings-NotificationContext-meinapetito" --query "value")
...
$portalDbConnectionString = "Server=tcp:stage-meinapetito.database.windows.net,1433;Initial Catalog=PortalDb;Persist Security Info=False;User ID=<userid>;Password=<value omitted>;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;" # somehow: grap app setting and replace placeholders for user / password from kv
...
# state für nservicebus secret importieren
terraform import -var-file="prod/env.tfvars" module.serviceBusNamespace.azurerm_key_vault_secret.meinapetito_azureservicebusnamespace_primaryconnectionstring "https://apetitokeyvault.vault.azure.net/secrets/apetito-meinapetito-azureservicebusnamespace-primaryconnectionstring/92b5a9aad1e3422f985623e32c4329fc"
...
cd cluster
./create-prod.ps1
./createIngressController-prod.ps1
